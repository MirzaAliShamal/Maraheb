...
class PurchaseManagerController extends Controller
{
    public function list(Request $req)
    {
        ...
        if ($req->ajax()) {
                    ...
                    if ($row->visibility_status) {
                        $html .= '
                            <div class="switchbox-outer">
                                <label class="switch">
                                    <input type="checkbox" data-url="'.route('recruiter.purchase.manager.status', $row->id).'" class="status" name="status" id="status_'.$row->id.'" checked>
                        ...
                        $html .= '
                            <div class="switchbox-outer">
                                <label class="switch">
                                    <input type="checkbox" data-url="'.route('recruiter.purchase.manager.status', $row->id).'" class="status" name="status" id="status_'.$row->id.'">
                                    <span class="slider round"></span>
                                    <span class="title"></span>
                                </label>
                            </div>
            ...
            return view('recruiter.purchase_manaager.list', get_defined_vars());
        }
    }

    public function add()
    {
        return view('recruiter.purchase_manaager.add', get_defined_vars());
    }

    public function edit($id = null)
    {
        return view('recruiter.purchase_manaager.edit', get_defined_vars());
    }
    ...
    public function save(Request $req, $id = null)
    {
        ...
        if (is_null($id)) {
            try {
                ...
                Mail::send('email.recruiter.purchase_manager_add', get_defined_vars(), function ($message) use($req) {
                ...
                return redirect()->route('recruiter.purchase.manager.list')->with('success', 'Purchase Manager registerd successfully!');
